...
void Arping::StartProcess() {
    ...
    // child process
    if (childPid == 0) {
        LOG(sls::logINFOBLUE) << "Created [ Arping Process, Tid: " << gettid()
                         << " ]";
        ProcessExecution();
    }
    // parent process
...
void Arping::StopProcess() {
    LOG(sls::logINFOBLUE) << "Exiting [ Arping Process ]";
...
std::string Arping::ExecuteCommands() {
    for (auto cmd : commands) {
        ...
        LOG(sls::logDEBUG1) << "Executing Arping Command: " << cmd;
